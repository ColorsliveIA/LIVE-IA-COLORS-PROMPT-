...
import { Zap, Sparkles, Headphones, Radio, ArrowRight, Brain, Cpu } from 'lucide-react';

interface HomeProps {
  onSelect: (view: 'studio' | 'suno', provider: 'gemini' | 'grok') => void;
}

export const Home: React.FC<HomeProps> = ({ onSelect }) => {
  return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
      {/* Background Atmosphere — dual glow */}
      ...
      <div className="max-w-6xl w-full z-10">
        <header className="text-center mb-16 md:mb-24">
          ...
          >
            Choose Your <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-white/20">AI Engine</span>
          </motion.h2>
          ...
          >
            Select your AI provider for Suno prompt generation.
            Each engine brings its own creative DNA.
          </motion.p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-3xl md:text-4xl font-bold mb-2 uppercase tracking-tight text-black">Gemini</h3>
              <div className="text-black/50 text-sm font-mono mb-4 uppercase tracking-widest">Google AI</div>
              <p className="text-black/60 text-lg mb-8 max-w-[280px]">
                Fast, structured generation with advanced style control and harmonic presets.
              </p>

              <div className="flex items-center gap-2 text-black font-bold uppercase tracking-widest text-sm">
                Start with Gemini <ArrowRight size={18} />
              </div>
            </div>
          ...
          <motion.button
            ...
            onClick={() => onSelect('suno', 'grok')}
            className="group relative bg-[#3B82F6] rounded-[32px] p-8 md:p-12 text-left overflow-hidden transition-all hover:shadow-[0_0_50px_rgba(59,130,246,0.3)]"
          >
            <div className="absolute top-0 right-0 p-8 opacity-20 group-hover:opacity-30 transition-opacity text-black">
              <Brain size={120} />
            </div>
            ...
            <div className="relative z-10">
              <div className="w-14 h-14 bg-black/20 rounded-2xl flex items-center justify-center mb-8 group-hover:bg-black group-hover:text-[#3B82F6] transition-colors text-black">
                <Cpu size={28} />
              </div>

              <h3 className="text-3xl md:text-4xl font-bold mb-2 uppercase tracking-tight text-black">Grok</h3>
              <div className="text-black/50 text-sm font-mono mb-4 uppercase tracking-widest">xAI Engine</div>
              <p className="text-black/60 text-lg mb-8 max-w-[280px]">
                Uncensored, raw street authenticity. No filters, no compromise on vocabulary.
              </p>

              <div className="flex items-center gap-2 text-black font-bold uppercase tracking-widest text-sm">
                Start with Grok <ArrowRight size={18} />
              </div>
            </div>

            {/* Decorative Grid */}
            <div className="absolute bottom-0 left-0 w-full h-1/2 bg-[linear-gradient(to_right,rgba(0,0,0,0.1)_1px,transparent_1px),linear-gradient(to_bottom,rgba(0,0,0,0.1)_1px,transparent_1px)] bg-[size:20px_20px] [mask-image:radial-gradient(ellipse_at_bottom_left,black,transparent_70%)] pointer-events-none" />
          </motion.button>
        </div>
